...
class Pyventa(QtGui.QMainWindow, Ui_Principal):
    def __init__(self):
        ...
        self.connect(self.tbRegistrar, QtCore.SIGNAL(
            "clicked()"), self.guardar)
        ...
        self.connect(self.actionAbrir_venta_extemporanea,
                     QtCore.SIGNAL("triggered()"), self.abrirVExtempo)
        self.connect(self.actionReimprimir_archivada, QtCore.SIGNAL(
            "triggered()"), self.imprimirArchivada)
        ...
        if self.cfg.stat:
            ...
            if self.conexion.stat:
                ...
                self.connect(self.completer, QtCore.SIGNAL(
                    "activated(const QModelIndex&)"), self.busca2)
    ...
    def setupActions(self):
        ...
        modific = menu.addAction(
            QIcon(":/actions/images/actions/color_18/calculator.png"), "Cambiar cantidad")
    ...
    def iniciarEstilo(self):
        try:
            kcss = open(os.path.join(self.raiz, "perfil", "estilos",
...
empresas.<p/> <p>Version %s<br/>Libreria visual: Qt4<br/>Plataforma: %s <br/> Desarrodador: David Osorio Hernandez\n\nPara mas informacion y actualizaciones: <a href=\"http://pyventa.com\">http://pyventa.com/</a></p>" % (sys.platform, version))
    ...
    def aut(self, nivel):
        ...
        if nivel <= self.sesion['usuario']['nivel']:
            ...
            key = str(Acceso[0])
            self.cursor.execute(
                "select id_usuario,nivel from usuarios where clave=MD5('{0}') ".format(key))
    ...
    def ingreso(self, tmp=False):
        # Se encarga de visualizar y agregar los datos de una tupla que
        # contenga un producto su cantidad y precio
    ...
    def descontar(self, producto):
        ...
        if rec != None:
            ...
            if promo != False and promo != None:
                ...
                self.Promo.setText("Aplica descuento de: {0}, {1}% ".format(
    ...
    def buscar(self, txt):
        if(txt != None):
            ...
            if ((len(txt) > 2) and (txt[0].isdigit() == False) and txt[0] != '!'):
                ...
                sql = "SELECT `descripcion`,`precio`,ref FROM productos where {0} order by vendidas desc  limit 20".format(
                    "and".join(fil))
    ...
    def reprint(self):
        nota = QtGui.QInputDialog.getInteger(self, self.tr(
            "Reimprimir venta"), self.tr("Numero de venta."))
        if nota[1]:
            self.limpiar()
            ide = nota[0]
            self.curser.execute(
    ...
    def grant(self):
        ...
        sub = round(suma, 1)
        self.ltotal.setText(
            """Gran total: <br/><span style="font-size:30pt;font-weight:600;">${0:,.2f}</span>""".format(total))
    ...
    def buscar_canasta(self, tupla):
        # Busca un producto dentro de la canasta de compra y regresa verdadero
        # si existe y falso en caso contrario
        ret = False
    ...
    def abrirVExtempo(self):
        ide = QtGui.QInputDialog.getInteger(self, self.tr(
            "Abrir venta archivada"), self.tr("Folio de venta:"))
        if ide[1]:
            try:
                sql = "select nota  from notas_cobradas where id={0} ;".format(
                    ide[0])
                self.cursor.execute(sql)
            except MySQLdb.Error, e:
                print e
    ...
    def abrirCompra(self, ide, edicion=True):
        ...
        try:
            ...
            sql = "SELECT compras.id,proveedor,nombre FROM compras,clientes WHERE proveedor=clientes.id and compras.id=%s " % (
                ide)
            # print sql
            self.cursor.execute(sql)
    ...
    def cancelNota(self):
        if self.aut(2) > 0:
            nota = QtGui.QInputDialog.getInteger(self, self.tr(
                "Cancelar nota"), self.tr("Numero de nota."))
            if nota[1]:
                try:
                    self.cursor.execute(
    ...
    def imprimirTicket(self, extraTags={}):
        ...
        for i, linea in enumerate(ticline):
            ...
            if linea.find("<importe/>") >= 0 or linea.find("<idesc/>") >= 0:
                # En la linea donde encuentre el importe lo toma como la
                # segunda linea
                dataf[1] = linea
                ticline.remove(linea)
            if linea.find("<producto/>") >= 0:
                # En la linea donde encuentre la etiqueta producto lo toma como
                # la primera linea
                dataf[0] = linea
                ticline[i] = "<producto/>"
        ticket = ''.join(ticline)
