...
if sys.platform == 'linux2':
    ...
    sys.stdout = open(
        os.path.join(os.path.expanduser('~'), "pyventa.salida.log"), "w")
    sys.stderr = open(
        os.path.join(os.path.expanduser('~'), "pyventa.error.log"), "w")
    home = os.path.join(os.path.expanduser('~'), "pyventa")
...
class Pyventa(QtGui.QMainWindow, Ui_Principal):
    ...
    def __init__(self):
        ...
        self.connect(self.actionIngresar,
                     QtCore.SIGNAL("triggered()"), self.insert)
        ...
        self.connect(self.actionAbrir_cajon,
                     QtCore.SIGNAL("triggered()"), self.abrirCajon)
        self.connect(self.actionModo_rapido,
                     QtCore.SIGNAL("toggled (bool)"), self.activarModo)
        self.connect(self.actionGuardarcompra,
                     QtCore.SIGNAL("triggered()"), self.guardarCompra)
        self.connect(self.actionAbrir_compra,
                     QtCore.SIGNAL("triggered()"), self.editarCompra)
        self.connect(self.actionNueva_compra,
                     QtCore.SIGNAL("triggered()"), self.nuevaCompra)
        self.connect(self.actionSwap, QtCore.SIGNAL("triggered()"), self.swap)

        self.connect(
            self.actionNueva, QtCore.SIGNAL("triggered()"), self.limpiar)
        ...
        self.connect(self.actionA_Pyventa,
        ...
        # self.connect(self.actionUpdate, QtCore.SIGNAL("triggered()"),
        # self.actualizar)
        self.connect(self.actionCopiar_nota,
                     QtCore.SIGNAL("triggered()"), self.copiarNota)
        self.connect(self.actionCancelar_nota,
                     QtCore.SIGNAL("triggered()"), self.cancelNota)
        ...
        self.connect(
            self.actionGuardar, QtCore.SIGNAL("triggered()"), self.savef)
        self.connect(self.actionFacturar, QtCore.SIGNAL(
            "triggered()"), self.cerrarFactura)
        self.connect(self.actionNueva_sesion,
                     QtCore.SIGNAL("triggered()"), self.iniciarSesion)
        self.connect(self.actionCerrar_sesion,
                     QtCore.SIGNAL("triggered()"), self.cerrarSesion)
        self.connect(self.actionCambiar_usuario,
                     QtCore.SIGNAL("triggered()"), self.iniciarSesion)
        self.connect(self.actionCompleta,
                     QtCore.SIGNAL("triggered()"), self.pantalla)
        self.connect(
            self.aConfigBd, QtCore.SIGNAL("triggered()"), self.configBd)
        ...
        # self.connect(self.lista2, QtCore.SIGNAL("keyPressEvent ( QKeyEvent *
        # event )"), self.drive)
        icon28 = QtGui.QIcon()
        icon28.addPixmap(
            QtGui.QPixmap(":/modulos/images/png/elegant/ventas.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        icon28.addPixmap(
            QtGui.QPixmap(":/modulos/images/png/elegant/ventas.png"), 2, QtGui.QIcon.Off)
        ...
        self.connect(
            self.aGuardar, QtCore.SIGNAL("triggered()"), self.guardar)
    ...
    def setupActions(self):
        ...
        self.removerProducto = menu.addAction(
            QIcon(":/actions/images/actions/black_18/cancel.png"), "Quitar de la lista", self.delete)
        self.tabla.addAction(modific)
        self.tabla.addAction(self.removerProducto)
        # self.menuHerramientas.addAction(QIcon(":/modulos/images/png/mini/productos.png"),"Checador
        # de precios",self.checador)
                        ...
                        # if self.cfg.has_option("mysql", "user"):
                                ...
                                # else:
                                    ...
                                    # if (nota!=None):
                                      ...
                                      # self.lnota.setText("Ultima Nota:
                                      # <b>#"+self.nota+"</b>")
    ...
    def iniciarMenus(self):
        ...
        action = self.popLista2.addAction(
            QtGui.QIcon(":/actions/images/actions/color_18/battery_empty.png"), "Marcar como faltante ", self.marcarFalta)
        ...
        try:
            kcss = open(
                os.path.join(os.path.dirname(os.path.realpath(__file__)),
                             "perfil", "estilos", self.cfg.get("pyventa", "estilo")), "r")
            estilo = kcss.read()
            ...
            try:
                kcss = open(
                    os.path.join(self.home, "estilos", self.cfg.get("pyventa", "estilo")), "r")
                estilo = kcss.read()
                self.setStyleSheet(estilo)
                kcss.close()
            except:
...
empresas.<p/> <p>Version %s<br/>Libreria visual: Qt4<br/>Plataforma: %s <br/> Desarrodador: David Osorio Hernandez\n\nPara mas informacion y actualizaciones: <a href=\"http://pyventa.com\">http://pyventa.com/</a></p>" % (sys.platform, version))
    ...
    def entablar(self, tabla, lista, head):
        # Cumple la misma funcion que tabular, con la diferencia que esta
    ...
    def loadStyle(self):
        openFile = QFileDialog.getOpenFileNames(
            self, "Seleccione el estilo", "/usr/share/pyventa/perfil", "Hojas de estilo (*.css)")
        if (openFile != ""):
            ...
            if txt[0] == '-':
                ...
                if codigo != None:
                    try:
                        ...
                        # print "SELECT `ref`,`descripcion`,`precio`  FROM
                        # productos as p, codigos as c where (ref=%s  OR
                        # p.codigo=%s OR c.codigo='%s') and producto=ref limit
                        # 1"%(codigo,codigo,codigo)
                    except MySQLdb.Error, e:
...
# Se encarga de visualizar y agregar los datos de una tupla que contenga
# un producto su cantidad y precio
        if (tmp == False):
            tmp = self.tutmp
            self.tutmp = False
    ...
    def buscar(self, txt):
        if(txt != None and self.tutmp == False):
            ...
            if ((len(txt) > 2) and (txt[0].isdigit() == False)):
                ...
                # self.lista2.resize(self.lista2.columnWidth(1)+self.lista2.columnWidth(2),
                # self.lista2.height() )
    ...
    def reprint(self):
        nota = QtGui.QInputDialog.getInteger(
            self, self.tr("Copiar nota"), self.tr("Ingrese el numero de nota."))
        if nota[1]:
            self.limpiar()
            ide = nota[0]
            self.curser.execute(
                'select * from notas where `id`=' + str(nota[0]))
            ...
            for i, prod in enumerate(prods):
                try:
                    ...
                    tmp = [float(prod), float(cant[i]), producto[
                        0], float(producto[1]), total]
                    self.ingreso(tmp)
            self.imprimirTicket()
            self.limpiar()
...
# Busca un producto dentro de la canasta de compra y regresa verdadero si
# existe y falso en caso contrario
        ret = False
    ...
    def guardarCompra(self):
        if self.aut(2) > 0:
            ...
            # QtGui.QMessageBox(QtGui.QMessageBox.Warning,"Seleccione un
            # proveedor","Busque y seleccione el proveedor de la compra
    ...
    def editarNota(self):
        if self.aut(2) > 0:
            ref = QtGui.QInputDialog.getText(
                self, self.tr("Editar nota"), self.tr("Numero de venta:"), QtGui.QLineEdit.Normal)
            if ref[1]:
    ...
    def editarCompra(self):
        if self.aut(2) > 0:
            ref = QtGui.QInputDialog.getText(
                self, self.tr("Editar compra"), self.tr("Numero de compra: "), QtGui.QLineEdit.Normal)
            if ref[1]:
        ...
        if nota[1]:
            ...
            for i, prod in enumerate(prods):
                try:
                    ...
                    producto = self.cursor.fetchone()
                    total = float(cant[i]) * float(producto[1])
                    tmp = [int(prod), float(cant[i]), producto[
                        0], float(producto[1]), total]
                    self.ingreso(tmp)
    ...
    def eliminarCompra(self, compra=-1):
        ...
        if self.aut(2) > 0:
            if compra == -1:
                dlg = QtGui.QInputDialog.getInteger(
                    self, self.tr("Cancelar Compra"), self.tr("Ingrese el numero de compra."))
                compra = dlg[0]
    ...
    def cancelNota(self):
        ...
        if self.aut(2) > 0:
            nota = QtGui.QInputDialog.getInteger(
                self, self.tr("Cancelar nota"), self.tr("Ingrese el numero de nota."))
            if nota[1]:
                try:
                    self.cursor.execute(
    ...
    def autDescuento(self):
        if self.aut(2) > 0:
            dsc = QtGui.QInputDialog.getDouble(
                self, self.tr("Autorizar descuento"), self.tr("Escriba el porcentaje de descuento que se aplicara"))
            dsc = dsc[0]
            if dsc > 50:
                msgBox = QtGui.QMessageBox(
                    QtGui.QMessageBox.Question, "El porcentaje de descuento es muy alto.<br> Desea continuar? ",
        ...
        if (openFile != ""):
            ...
            for prod in items:
                ...
                try:
                    ...
                    tmp = [rec[0], p[1], rec[1], rec[
                        2], (float(p[1]) * float(rec[2]))]
                    self.ingreso(tmp)
            out.close()
...
# Guarda en un archivo la nota, pero no la asienta en la base de dtos
    ...
    def presupuestar(self):
        ...
        html = listaHtml(
            self.basket, titulo="Lista de productos", cabezas='Ref,Cantd,Descripcion del Producto,Precio,Importe,Subtotal', fondo='#000',
    ...
    def selCliente(self):
        ...
        app = Selector(
            self, titulo, 'clientes', 'id,nombre,rfc', 'Id,Nombre ,RFC',
