...
class acceso(QtGui.QDialog, Ui_Acceso):
    def __init__(self):
		...
		self.pbAceptar.setDefault(True)
        	...
        	self.logo.setPixmap(QtGui.QPixmap(":/actions/images/padmin.simple.logo.png"))
    ...
    def conexion(self):
		...
		if self.check():
			...
			try:
			    ...
			    usuario = self.cfg.get("mysql", "user")  
			    password = base64.b64decode(self.cfg.get("mysql", "pass"))
			    data = self.cfg.get("mysql", "db")
			    self.db = MySQLdb.connect(host, usuario, password,data)
			except:
	...
	if len(usuario)>0:
	  ...
	  try:
		...
		self.done(int(usuario['id_usuario']))
