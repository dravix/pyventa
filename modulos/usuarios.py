...
class Usuarios:
    def __init__(self,parent):
                ...
                self.parent=parent
                self.ui.twUsuarios.setContextMenuPolicy(QtCore.Qt.ContextMenuPolicy.CustomContextMenu)
                self.index=8
    ...
    def iniciar(self):
        for key,item in self.nivel.items():
    ...
    def eliminar(self):
        msgBox=QMessageBox(QMessageBox.Question,"Confirmar dar de baja?","Esta a punto de eliminar un usuario .Desea continuar?",QMessageBox.Yes|QMessageBox.No,self.ui, QtCore.Qt.WindowType.WindowStaysOnTopHint)
    ...
    def editar(self,y,x):
        ...
        if usuario!=None:
          ...
          self.ui.cbNivel.setCurrentIndex(self.ui.cbNivel.findData(usuario['nivel']))
          #self.ui.wuControls.setVisible(True)
...
class editorUsuarios(QDialog, Ui_Dialog):
    ...
    def iniciar(self):
        ...
        if self.ide!=-1:
          ...
          self.cbNivel.setCurrentIndex(self.cbNivel.findData(usuario['nivel']))
...
class editorClaves(QDialog, Ui_Claves):
    def __init__(self,parent,ide):
                ...
                self.time.timeout.connect(self.limpiarAlerta)
