...
class Productos:
    def __init__(self, parent):
        ...
        self.anterior = []
        self.alternos = []
        self.banderas = {'retorno': 1, 'anterior': [], 'siguiente': [],
            'editor': 'e'}  # Editor:n nuevo, e edicion, m edicion masiva
        self.tmpRef = 0
        self.modelo = None

    def setupSignals(self):
        self.parent.tblProductos.setContextMenuPolicy(Qt.CustomContextMenu)
        self.parent.connect(self.parent.verProductos,
                            SIGNAL("triggered()"), self.goProductos)
        self.parent.connect(self.parent.tProductos,
                            SIGNAL("clicked()"), self.goProductos)
        self.parent.connect(self.parent.tbProductos,
                            SIGNAL("clicked()"), self.goProductos)
        self.parent.connect(self.parent.tbpEditor, SIGNAL(
            "clicked()"), lambda: self.parent.stkProductos.setCurrentIndex(0))
        self.parent.connect(self.parent.tblProductos, SIGNAL(
            'customContextMenuRequested(const QPoint)'), self.ocmProductos)
        self.parent.connect(self.parent.tbpBuscar, SIGNAL("clicked()"), self.buscar)
        self.parent.connect(self.parent.tbpSiguiente, SIGNAL(
            "clicked()"), self.editarSiguiente)
        self.parent.connect(self.parent.tbpAnterior, SIGNAL(
            "clicked()"), self.editarAnterior)
        self.parent.connect(self.parent.tbpNuevo, SIGNAL(
            "clicked()"), self.nuevoProducto)
        self.parent.connect(self.parent.tbpEliminar, SIGNAL(
            "clicked()"), self.eliminarProducto)
        self.parent.connect(self.parent.tbpAgregarFamilia, SIGNAL(
            "clicked()"), self.parent.familia.iniciar)
        self.parent.connect(self.parent.tbpAgregarImpuesto, SIGNAL(
            "clicked()"), self.parent.impuesto.iniciar)
        self.parent.connect(self.parent.tbpAgregarUnidad, SIGNAL(
            "clicked()"), self.parent.unidad.iniciar)
        self.parent.connect(self.parent.tbpAgregarStock,
                            SIGNAL("clicked()"), self.agregarStock)
        self.parent.connect(self.parent.lepFiltro, SIGNAL(
            "returnPressed()"), self.buscar)
        self.parent.connect(self.parent.tpcodigo, SIGNAL(
            "returnPressed()"), self.checkCodigo)
        self.parent.connect(self.parent.leProd, SIGNAL(
            "returnPressed()"), lambda: self.buscarProducto(self.parent.leProd))
        ...
        self.parent.connect(self.parent.tblProductos, SIGNAL(
            "activated(const QModelIndex&)"), self.editarProductos)
        self.parent.connect(self.parent.tbpRestaurar, SIGNAL(
            "clicked()"), lambda: self.editarProducto(self.tmpRef))
        self.parent.connect(self.parent.tbpGuardar, SIGNAL(
            "clicked()"), self.editarProducto_guardar)
        self.parent.connect(self.parent.tpgana, SIGNAL(
            "editingFinished()"), self.calcprecio)
        self.parent.connect(self.parent.tbListarCodigos,
                            SIGNAL("clicked()"), self.listarCodigos)
    ...
    def setTools(self):
        ...
        self.popMenu.addAction(QIcon(
            ":/actions/images/actions/black_18/coin.png"), "Cambiar precios", self.cambiarPrecios)
        self.popMenu.addAction(QIcon(":/actions/images/actions/black_18/battery_empty.png"),
                                "Marcar como faltante", self.marcarFalta)
        self.popMenu.addAction(QIcon(":/actions/images/actions/black_18/delete.png"),
                                "Eliminar producto", self.eliminarProducto)
    ...
    def buscar(self):
        ...
        if len(txt) > 0:
            ...
            if (txt[0].isalpha()):
                #Separa cada palabra y la busca en la base de datos
                li = txt.split(' ')
                fil = []
                for l in li:
                    fil.append(" descripcion like '%{0}%' ".format(l))
                tipo = "and".join(fil)
            # En caso que sea un digito lo busca como referencia o como codigo de barras
            elif txt[0].isdigit():
                tipo = "((P.codigo={0} OR ref={0})or ref=(select producto from codigos as c where c.codigo={0}))".format(
                txt)
            elif txt[0] == '*':
                tipo = '1'
            else:
                return True
            head = ['Ref', 'Codigos', 'Descripcion', 'Famlia', 'Cost', 'Gan',
                'Prcio', 'Exist', 'Unidad', 'Impto', 'Ventas', 'U.Modifica']
            ...
            sql = "SELECT {0} FROM productos as P, familias ,existencia , unidades, impuestos where existencia.producto=ref and unidades.id=unidad and familias.id=familia and impuestos.id=impuesto and {1}  group by ref order by `descripcion`; ".format(
                campos, tipo)
            try:
            ...
            self.parent.setStatus(
                0, "Mostrando %s resultados en la tabla." % self.modelo.getRowCount())
	...
	msgBox = QMessageBox(QMessageBox.Question, "Confirmar eliminacion", "Confirma eliminar los productos seleccionados? Esta operacion no puede deshacerse.",
	                     QMessageBox.Yes | QMessageBox.No, self.parent, Qt.WindowStaysOnTopHint)
	if msgBox.exec_() == QMessageBox.Yes:
	  if self.tmpRef > 0:
	    ides = [int(self.tmpRef)]
	  else:
	    indexes = self.parent.tblProductos.selectedIndexes()
	    ides = []
	  ...
	  for ref in ides:
	    if ref > 0:
	      self.cursor.execute(
	          "select count(producto) from vendidos where producto=%s" % ref)
	      vendidos = self.cursor.fetchone()
	      if int(vendidos[0]) > 0:
		msgBox = QMessageBox(QMessageBox.Critical, "Advertencia!",
		                     "El producto no puede ser eliminado porque ya existen ventas ligadas a el.", QMessageBox.Close, self.parent)
		msgBox.exec_()
	      else:
		try:
		    ...
		    pass
	  QMessageBox.information(self.parent, "Producto(s) eliminado(s)",
	                          "La operacion ha finalizado correctamente.")
	  self.buscar()
	...
	# self.cursor.execute()
	self.parent.entabla(self.parent.tvpPrecios, [
	                    'Promocion', 'Apartir de', '%', 'P.Unitario', 'P.Total'], sql)
	...
	sql = "select P.*,D.nombre as dep,F.nombre as fam, stock_logico  from productos as P,departamentos as D, familias as F, existencia where producto=ref and P.familia=F.id and P.ref={0} ;".format(
	    ref)
	...
	if producto != None:
	  ...
	  # Pone el combo en el valor del Id
	  setComboModelKey(self.parent.cbeFamilias, producto['familia'])
	  ...
	  # Pone el combo en el valor del Id
	  setComboModelKey(self.parent.cbeImpuestos, producto['impuesto'])
	  # Pone el combo en el valor del Id
	  setComboModelKey(self.parent.cbeUnidades, producto['unidad'])
	  ...
	  try:
	    self.curser.execute(
	        "SELECT *,count(subproducto)as num FROM subproductos where subproducto="+str(ref))
	...
	if producto['codigo'] == '':
		msgBox = QMessageBox(QMessageBox.Question, "Falta el codigo de barras",
		                     "No ha escrito ningun codigo de barras, es necesario un codigo unico.<br>Desea que se genere automaticamente?", QMessageBox.Yes | QMessageBox.No, self.parent)
		ret = msgBox.exec_()
	  ...
	  try:
		...
		if prev != None:
		    QMessageBox.critical(self.parent, "Codigo duplicado",
		                         "Este codigo ya fue asignado a\n%s." % prev[0])
		    return False
		else:
		    self.cursor.execute("INSERT INTO productos VALUES(%s,NULL,'%s',%s,%s,%s,%s,%s,%s,0,%s,NOW())" % (
		        producto['codigo'], producto['descripcion'], producto['familia'], producto['costo'], producto['ganancia'], producto['precio'], producto['stock'], producto['unidad'], producto['impuesto']))
	  except MySQLdb.Error, e:
                mb = QMessageBox.critical(
                    self.parent, "Error!", "No se ha podido ingresar. Verifique los datos")
                self.parent.setStatus(
                    2, "No se ha podido ingresar. Verifique los datos")
                print e
		...
		self.cursor.execute("INSERT INTO existencia VALUES(%s,0,0,0,0)" % pd[0])
                mb = QMessageBox.information(
                    self.parent, "El producto ha sido dado de alta", ".Su referencia es: %s " % pd[0])
                self.parent.setStatus(
                    1, "El producto ha sido dado de alta. Su referencia es: {0}".format(pd[0]))
                if self.parent.gbRelacion.isChecked() == True:
		    sub = {'cantidad': str(self.parent.leNum.value()),
		                           'producto': str(self.parent.leProd.text())}
		    self.cursor.execute("""INSERT INTO subproductos VALUES( %s,%s,%s)""" % (
		        pd[0], sub['cantidad'], sub['producto']))
		    self.parent.conexion.commit()
		self.limpiarEditor()
		# self.goProductos()
	...
	producto['familia'] = "'%s'" % self.parent.cbeFamilias.model().celda(
	    self.parent.cbeFamilias.currentIndex(), 0)
	...
	producto['stock'] = "'%s'" % (
	    self.parent.tstock.value()+self.parent.dsbStock.value())
	producto['impuesto'] = "'%s'" % self.parent.cbeImpuestos.model().celda(
	    self.parent.cbeImpuestos.currentIndex(), 0)
	producto['unidad'] = "'%s'" % self.parent.cbeUnidades.model().celda(
	    self.parent.cbeUnidades.currentIndex(), 0)
	...
	if self.parent.tpcodigo.text() != '':
	    ...
	    try:
	      ...
	      self.cursor.execute(str(sql))
                ...
                self.parent.setStatus(
                    2, "El producto no pudo ser actualizado correctamente, verifique los datos")
                print e
                self.pbStat.setToolTip("%s" % e)
		msgBox = QMessageBox(QMessageBox.Critical, "Error",
		                     "El producto no pudo ser actualizado correctamente, verifique los datos.", QMessageBox.Close, self.parent)
		msgBox.exec_()
	    else:
	...
	if self.parent.gbRelacion.isChecked() == True:
	  	sub={'cantidad': str(self.parent.leNum.value()),
	  	                     'producto': str(self.parent.leProd.text())}
	  	self.cursor.execute(
	  	    "select count(subproducto) from subproductos where subproducto="+str(self.tmpRef))
	  	num=self.cursor.fetchone()[0]
	  	if num > 0:
		    self.cursor.execute("""UPDATE subproductos set cantidad=%s, producto=%s where subproducto=%s""" % (
		        sub['cantidad'], sub['producto'], self.tmpRef))
		    self.parent.conexion.commit()
		else:
		    self.cursor.execute("""INSERT INTO subproductos VALUES( %s,%s,%s)""" % (
		        self.tmpRef, sub['cantidad'], sub['producto']))
		    self.parent.conexion.commit()
    ...
    def agregarStock(self):
      dsc=QInputDialog.getDouble(self.parent, self.parent.tr(
          "Ajustar stock."), self.parent.tr("Unidades adicionales:"))
      dsc=dsc[0]
      if dsc != 0 and self.tmpRef > 0:
	print "Agregando %s piezas " % (dsc)
	try:
	  self.cursor.execute(
	      'UPDATE existencia set stock_logico=stock_logico+%s WHERE producto=%s' % (dsc, self.tmpRef))
	except:
    ...
    def cambiarPrecios(self):
      indices=self.parent.tblProductos.selectedIndexes()
      productos={}
      for index in indices:
	if index.column() == 0:
	  productos[index.row()]=[str(index.data().toString())]
	elif index.column() in (2, 4, 5, 6):
	  productos[index.row()].append(str(index.data().toString()))
      productos=[item for key, item in productos.iteritems()]
	...
	sql=" select %s from productos as p where p.codigo=%s OR ref=%s or (ref=(select producto from codigos as c where c.codigo=%s));" % (
	    campos, codigo, codigo, codigo)
	# print sql
	self.cursor.execute(sql)
