...
class Admin1(QtWidgets.QDialog,Form):
...
#       Ide:    En el caso de que le pasen ide significa que editara un objeto en particular y solo se realizara una accion (modificar, agregar)
              ...
              for key, item in enumerate(campos):
                ...
                if self.campos[key][3]==None:
                  ...
                  if item[2]=='hide':
                    ...
                    self.campos[key][3]=QtWidgets.QLineEdit(self)
                    self.campos[key][3].setAlignment(QtCore.Qt.AlignmentFlag.AlignLeft)
                  elif item[2]=='pass':
                    self.campos[key][3]=QtWidgets.QLineEdit(self)
                    self.campos[key][3].setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
                    self.campos[key][3].setEchoMode(2)
                  elif item[2]=='int':
                    self.campos[key][3]=QtWidgets.QSpinBox(self)
                    self.campos[key][3].setAlignment(QtCore.Qt.AlignmentFlag.AlignRight)
                    self.campos[key][3].setButtonSymbols(2)
                    self.campos[key][3].setMaximum(10000)
                    self.campos[key][3].setMinimum(0)
                  elif item[2]=='double':
                    self.campos[key][3]=QtWidgets.QDoubleSpinBox(self)
                    self.campos[key][3].setAlignment(QtCore.Qt.AlignmentFlag.AlignRight)
                    self.campos[key][3].setButtonSymbols(2)     
                    self.campos[key][3].setMaximum(10000)
                    self.campos[key][3].setMinimum(0)               
                  elif item[2]=='date':
                    self.campos[key][3]=QtWidgets.QDateEdit(self)
              ...
              ### -------------------------------------
              self.tvTabla.setContextMenuPolicy(QtCore.Qt.ContextMenuPolicy.CustomContextMenu)
              ...
              self.tbHome.clicked.connect(self.listar)
              self.tbModif.clicked.connect(self.guardar)
              self.tbAgregar.clicked.connect(self.limpiar)
              self.tbLimpiar.clicked.connect(self.limpiar)
              #self.connect(self.tvTabla,QtCore.SIGNAL('cellDoubleClicked (int, int)'),self.seleccionar)
              self.tvTabla.activated.connect(self.seleccionar)
              self.tvTabla.customContextMenuRequested.connect(self.ocm)
              self.cargarMenu()       
              #self.anclar()
              self.editando=False
              self.listar()
    ...
    def eliminar(self,ide=-1):
        ...
        for ide in ides:
          if int(ide)>1:
            ...
            msgBox=QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Information,"No se puede eliminar","No se puede eliminar este elemento porque es el registro por default.",QtWidgets.QMessageBox.StandardButton.Ok,self.padre)
            msgBox.exec()
