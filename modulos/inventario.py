...
class Inventario:
    def __init__(self,parent):
        ...
        self.menuPop = QtWidgets.QMenu(self.ui)
...
class AsistInventario(QtWidgets.QDialog, Ui_Asistente):  
    def __init__(self,parent,usuario=-1):
                ...
                self.modo=0
                #insert=QtWidgets.QAction(self)
                #insert.setShortcut("Ins")
                self.tbAdelante.clicked.connect(self.continuar)
                self.connect(self.tbManual, QtCore.SIGNAL("stateChanged ( int  )"), self.manual)
                
                self.tbAtras.clicked.connect(self.retroceder)
                self.tbCancelar.clicked.connect(self.cancelar)
                self.tbSigProd.clicked.connect(self.sigProd)
                self.tbAntProd.clicked.connect(self.antProd)
                self.tbSetStock.clicked.connect(self.setStock)
                self.tbImprimir.clicked.connect(self.imprimir)
                self.tbIncon.clicked.connect(self.listarIncon)
                self.tabla.activated.connect(self.seleccionar)
                self.tbPrintInc.clicked.connect(self.printInc)
                
                #self.timer.timeout.connect(self.autoGuardar)
                self.connect(self.leExistencias, QtCore.SIGNAL("returnPressed ()"), self.setStock)
                self.leFiltro.textChanged.connect(self.buscar)
                self.leFiltro.returnPressed.connect(self.contar)
                self.inicio()
    ...
    def inicio(self):
        ...
        if res!=None:
          msgBox=QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Question,"Inventario en curso","Actualmente hay un inventario en proceso de conteo.<h3>Desea participar en este conteo?</h3>",QtWidgets.QMessageBox.StandardButton.Yes|QtWidgets.QMessageBox.StandardButton.No,self)
    ...
    def continuar(self):
        ...
        if (current+1==1):                      #Si es el primer paso selecciona los departamentos
          ...
          for val,radio in self.ordenes.items():    #Por cada radiobutton si este esta checado toma su valor para ordenar
...
#Al pasar a la etapa de conteo    
          ...
          self.lbStatus.setText("Etapa de conteo")        
          msgBox=QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Question,"Confirmacion","A continuacion se procedera a contar y no sera posible cancelar o editar las configuraciones del inventario hasta que se de por terminado.<h3>Desea continuar?</h3>",QtWidgets.QMessageBox.StandardButton.Yes|QtWidgets.QMessageBox.StandardButton.No,self)
          if msgBox.exec()==QtWidgets.QMessageBox.StandardButton.Yes:
            self.tbCancelar.setEnabled(False)
            self.tbAtras.setEnabled(False)
            try:
...
#Al terminar de contar      
          ...
          msgBox=QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Question,"Confirmacion","En este punto, todos los productos deben estar contabilizados, y se procedera a cerrar el inventario.<h3>Desea seguro que desea terminar el inventario?</h3>",QtWidgets.QMessageBox.StandardButton.Yes|QtWidgets.QMessageBox.StandardButton.No,self)
          self.gerente=self.autentificacion(2)
          if msgBox.exec()==QtWidgets.QMessageBox.StandardButton.Yes and self.gerente!=False:       
            self.stack.setCurrentIndex(self.stack.currentIndex()+1)
            self.setCursor(QtGui.QCursor(16))
