...
class ListadorCodigos(QDialog, Ui_Listado):
    def __init__(self,parent,ide=-1,lista=[]):
      ...
      self.modelo=None
      self.codes=lista
      self.tbMas.clicked.connect(self.nuevo)    
      self.tbMenos.clicked.connect(self.eliminar)    
      #if self.ide>0:
      self.listar()
      #else:
    ...
    def eliminar(self):
      #msgBox=QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Question,"e ",QtWidgets.QMessageBox.StandardButton.Yes|QtWidgets.QMessageBox.StandardButton.No,self, QtCore.Qt.WindowType.WindowStaysOnTopHint)
      #ret=msgBox.exec()
