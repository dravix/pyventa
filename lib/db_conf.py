...
<li>Averigue si el servidor esta disponible en red</li></ol>')
                ...
                self.display.setText("<h1>Error al intentarse conectar a la BD.</h1>\
 </br> <p>%s</p>"%'<br/>'.join(e.args))
        except Exception as e:
            raise(e)
            self.display.setText(
                '<h1>Error desconocido.</h1><p>Verifique el registro de errores para conocer mas</p>')
        else:
            self.bset.setEnabled(True)
            self.bcreate.setEnabled(True)
            self.display.setText(
                '<h1>Conectado.</h1><p>Guarde esta configuracion para que Pyventa se conecte usando esta base de datos</p>')
            self.host = str(self.tserver.text())
            self.user = str(self.tuser.text())
            self.password = str(self.tpass.text())
            self.db = base
