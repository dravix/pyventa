...
if sys.platform == 'linux2':
  import cups
class Configs(QtWidgets.QDialog, Ui_Form):
    def __init__(self,parent,id=0):
        ...
        icono = QtGui.QIcon()
        icono.addPixmap(QtGui.QPixmap(":/modulos/images/png/elegant/config.png"), 0, QtGui.QIcon.State.Off)
        ...
        self.cbPath.clicked.connect(self.cambiarFolderFacturas)
        self.ctPrinter.clicked.connect(self.cambiarImpresoraTickets)
        self.pbEditar.clicked.connect(self.editar)
        
        #self.connect(parent.stack, QtCore.SIGNAL("currentChanged(int)"),lambda: parent.aut(self.id,2) )
        #self.tablaImpuestos.currentItemChanged.connect(self.cambiarImp)
        #parent.stack.currentChanged.connect(self.inicia)
        self.rlBackup.clicked.connect(self.respaldarLocal)
        self.cpgrespaldo.clicked.connect(self.respaldarRemoto)
        ...
        self.cbEstilos.activated.connect(self.cambiarEstilo)
        self.cbPrinters.activated.connect(self.setPrinter)
        self.cbDrivers.activated.connect(self.setDriver)
        self.clrExplorar.clicked.connect(self.cambiarFolderRespaldo)
        self.cbpreview.clicked.connect(self.editarTicket)
        self.pbfEditar.clicked.connect(self.editarFactura)
        self.pbpEditar_.clicked.connect(self.editarPresupuesto)
        self.pbEditarCorte.clicked.connect(self.editarCorte)
        self.rlRestore.clicked.connect(self.restaurar)
        ...
        self.tbBuscarCaja.clicked.connect(self.buscador)
        self.tbLogo.clicked.connect(self.cambiarLogo)
        self.tbRecargarEstilo.clicked.connect(self.parent.iniciarEstilo)
        ...
        #self.connect(self.gbFacturas,QtCore.SIGNAL("clicked()"),lambda: self.setCambio('facturas','default',self.boolint(self.gbFacturas.isChecked())))                #self.bfSave.clicked.connect(self.cambiarFolderFacturas)
...
<li>Averigue si el servidor esta disponible en red</li></ol>')
    ...
    def respaldarLocal(self):
      if self.parent.aut(2)>0:
        ...
        msgBox=QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Information,"El respaldo fue generado.","Se ha creado el respaldo  %s."%out,QtWidgets.QMessageBox.StandardButton.Close,self)
        msgBox.exec()
        return out
    ...
    def restaurar(self,database=True,config=True):
          ...
          if (saveFile!=""):
            ...
            if rs.restaurar(saveFile,database,config):
                ...
                msgBox=QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Information,"El respaldo ha sido restaurado.","La base de datos ha sido restaurada, todos los cambios hechos desde la fecha del respaldo, han sido eliminados.",QtWidgets.QMessageBox.StandardButton.Close,self)
                msgBox.exec()
