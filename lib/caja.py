...
class Cajas(QtWidgets.QDialog, Ui_Form):
    def __init__(self,parent,id):
                ...
                icon28 = QtGui.QIcon()
                icon28.addPixmap(QtGui.QPixmap(":/modulos/images/png/elegant/sales.png"), 0, QtGui.QIcon.State.Off)
                icon28.addPixmap(QtGui.QPixmap(":/modulos/images/png/elegant/sales.png"), 2, QtGui.QIcon.State.Off)
                self.action.setIcon(icon28)
                self.action.setIconVisibleInMenu(True)
                self.action.setText(self.datos['nombre'])
                self.agasto=QtWidgets.QAction(self)
                ...
                copy=QtWidgets.QAction(QtGui.QIcon(":/actions/images/actions/black_18/copy.png"),"Copiar",self)
                copy.triggered.connect(self.teEntradasDetalle.copy)

                self.teEntradasDetalle.addAction(copy)
                self.tboImprimir.clicked.connect(self.imprimir)
                self.tboActualizar.clicked.connect(self.actualizar)
                self.agasto.triggered.connect(self.gastar)
                self.tboInicial.clicked.connect(self.inicial)
                self.pbDepositar.clicked.connect(self.depositar)
                self.tbRetirar.clicked.connect(self.retirar)
                self.pbAgregarGasto.clicked.connect(self.gastar)
                self.pbDetallarProds.clicked.connect(self.detallarProds)
                self.connect(self.pbVerResumen, QtCore.SIGNAL("clicked()"),lambda: self.stack.setCurrentIndex(0))
                self.action.triggered.connect(self.iniciar)
                
                self.pbVerEnDetallada.clicked.connect(self.detallarEntradas)
                self.tboDetalles.clicked.connect(self.detallarEntradas)
                self.tbGraficar.clicked.connect(self.graficar)
