...
from PyQt6.QtCore import QTimer, SIGNAL
...
class Cventa(QtWidgets.QDialog, Ui_Form):
    def __init__(self,parent,id):
                ...
                icon28 = QtGui.QIcon()
                icon28.addPixmap(QtGui.QPixmap(":/modulos/images/png/elegant/check.png"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
                self.action.setIcon(icon28)
                self.action.setIconVisibleInMenu(True)
                self.action.setText(self.datos['nombre'])
                
                #icon28.addPixmap(QtGui.QPixmap("/usr/share/pyventa/images/png/%scventa.png"%parent.iconset), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
                #self.action.setIcon(icon28)
                #self.action.setIconVisibleInMenu(True)
                self.action.triggered.connect(self.ver)
                self.imp=True
                self.parent.actionCerrar_nota.triggered.connect(self.ver)
    ...
    def guardarPresupuesto(self):
        paper=QtWidgets.QGraphicsScene()
...
#Almacena en la base de datos, l nota con todos sus elementos
      ...
      if len(self.parent.basket)>0:
        ...
        if  not ret:
              mb=QtWidgets.QMessageBox.critical(self, "Error!", "La nota no pudo ser registrada !!")
    ...
    def eliminarNota(self,nota):
      if self.parent.aut(2):
        try:
          ...
          QtWidgets.QMessageBox.critical(self, "Error!", "No fue posible eliminar la nota")
        else:   
          self.parent.conexion.commit()
          QtWidgets.QMessageBox.informative(self, "Error!", "La nota no pudo ser registrada !!")
