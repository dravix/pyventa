...
def setIcono(widget,icono):
        icon= QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(icono), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        widget.setIcon(icon)
...
class editorSimple(QtWidgets.QDialog, Editor):
    def __init__(self,parent, archivo):
        QtWidgets.QDialog.__init__(self)
        self.setupUi(self)
        self.parent=parent
        self.archivo=archivo
        self.accepted.connect(self.guardar)
        f=open(archivo,'r+')
        ticket=f.read()
        f.close()
...
class Documento:
    ...
    def iniciar(self):
      ...
      for key,item in self.campos.items():
          self.plantilla=self.plantilla.replace(key,item)
      #alto=765
      #ancho=765
      #self.escena.setSceneRect(0,0,ancho,alto)
      droid=QtGui.QFont("Droid Sans", 8, QtGui.QFont.Weight.Bold)
    ...
    #def imprimir(self):
        ...
        #prev=QtPrintSupport.QPrintDialog(printer,self.padre)
...
class AperturaCaja(QtWidgets.QDialog, Apertura):
    def __init__(self,parent=None):
                QtWidgets.QDialog.__init__(self,parent)
                self.setupUi(self)
                self.cursor=parent.cursor
                self.parent=parent
                self.accepted.connect(self.setInicial)  
...
class Seguridad(QtWidgets.QDialog, Ui_Acceso):
    def __init__(self,parent=None):
                QtWidgets.QDialog.__init__(self,parent)
                #self.con=conexion()
                ...
                self.reject.connect(self.getUser)
...
class VisorNotas(QtWidgets.QDialog, Ui_Resumen):
...
class ResumenVenta(QtWidgets.QDialog, Ui_RVenta):
...
class Factura:
    def __init__(self,padre,num):
        ...
        self.num=num
        self.escena=QtWidgets.QGraphicsScene()
        self.gvDocumento=QtWidgets.QGraphicsView()
        self.iniciar()
    ...
    def alinear(self,item):
      ...
      formato.setAlignment(QtCore.Qt.AlignmentFlag.AlignRight)
...
#class Documento(QtWidgets.QDialog, Ui_Form):       
